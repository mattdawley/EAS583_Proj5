...
def scanBlocks(chain):
    ...
    for block_num in range(start_block, end_block + 1):
        if chain == 'source':
            ...
            for evt in events:
                dest_contract.functions.wrap(evt.args['token'], evt.args['recipient'], evt.args['amount'])
            ...
            events = event_filter.get_all_entries()
            for evt in events:
                src_contract.functions.withdraw(evt.args['token'], evt.args['recipient'], evt.args['amount'])
